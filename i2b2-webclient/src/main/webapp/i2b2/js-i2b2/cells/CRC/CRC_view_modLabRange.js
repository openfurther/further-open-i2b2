...
i2b2.CRC.view.modalLabValues = {
	...
	cfgTestInfo: {
		name: 'RND-TEST',
		flagType: 'NA',
		flags: [{name:'Abnormal', value:'A'},{name:'Normal', value:'N'}],
		valueValidate: {
			onlyPos: true,
			onlyInt: true,
			maxString: 0 
	...
	cfgByMetadata: function(refXML){
		...
		try { 
			...
			if (t) {
				if (t == "A") {
					dm.flagType = 'NA';
					dm.flags = [{name:'Normal', value:'N'},{name:'Abnormal', value:'A'}];
				} else if (t == "HL") {
					dm.flagType = 'HL';
					dm.flags = [
...
// ================================================================================================== //
	ValidateSave: function() {
		...
		switch (fd.selectedType) {
			case "NONE":
// FUR-465: fix for the case of a non-null lab value in a stored query that is set to
// "No Value" in the lab value menu. We do this by also deleting the stored query's lab value,
// so that it is not used as a fall-back value in CRC_ctrlr_QryTool.js upon running the query.
				delete i2b2.CRC.view.modalLabValues.i2b2Data.origData.LabValues;
