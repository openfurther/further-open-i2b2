...
function QueryToolController() {
	...
	this.doQueryClear = function() {
		...
		delete i2b2.CRC.model.associatedResults
		i2b2.CRC.model.queryCurrent = {};
		$("assocResultBox").innerHTML = "Drop a previous result set on here"
	...
	// ==================================================================================================
	// //
	this.doQueryLoad = function(qm_id) { // function to load query from
											// history
		...
		scopedCallback.callback = function(results) {
			...
			if (qd.length != 0) {
				...
				dObj.specificity = i2b2.h.getXNodeVal(qd[0],
						'specificity_scale');
				dObj.panels = [];
				...
				for ( var i1 = 0; i1 < total_panels; i1++) {
					...
					po.exclude = (t == "1");
					var t = i2b2.h
							.getXNodeVal(qp[i1], 'total_item_occurrences');
					po.occurs = (1 * t) - 1;
					...
					for (i2 = 0; i2 < pi.length; i2++) {
						...
						if (c.length > 0) {
							...
							o.visual_attribs = i2b2.h.getXNodeVal(c,
									'visualattributes');
							o.hasChildren = i2b2.h.getXNodeVal(c,
									'visualattributes').substring(0, 2);
							// these are not needed?
							o.fact_table_column = i2b2.h.getXNodeVal(c,
									'facttablecolumn');
							o.column_name_datatype = i2b2.h.getXNodeVal(c,
									'columndatatype');
							o.synonym_cd = i2b2.h.getXNodeVal(c, 'synonym_cd');
							o.totalnum = i2b2.h.getXNodeVal(c, 'totalnum');
							// Lab Values processing
							var lvd = i2b2.h.XPath(pi[i2],
									'descendant::constrain_by_value');
							if (lvd.length > 0) {
								lvd = lvd[0];
								// pull the LabValue definition for concept
								var lvdef = i2b2.h
										.XPath(c,
												"descendant::metadataxml/ValueMetadata[Loinc]");
								if (lvdef.length > 0) {
									lvdef = lvdef[0];
								} else {
									lvdef = false;
								...
								// extract & translate
								var t = i2b2.h.getXNodeVal(lvd,
										"value_constraint");
								o.LabValues = {};
								o.LabValues.NumericOp = i2b2.h.getXNodeVal(lvd,
										"value_operator");
								o.LabValues.GeneralValueType = i2b2.h
										.getXNodeVal(lvd, "value_type");
								...
								case "TEXT": // this means Enum?
									o.LabValues.MatchBy = "VALUE";
									try {
										o.LabValues.ValueEnum = eval("(Array"
												+ t + ")");
									} catch (e) {
										console
												.error("Conversion Failed: Lab Value data = "
														+ t);
									}
									break;
								case "FLAG":
							...
							// sdx encapsulate
							var sdxDataNode = i2b2.sdx.Master.EncapsulateData(
									'CONCPT', o);
							if (o.LabValues) {
								// We do want 2 copies of the Lab Values: one is
								// original from server while the other one is
							...
							console
									.error("CRC's ONT Handler could not get term details about '"
											+ ckey + "'!");
						}
					}
				...
				$('infoQueryStatusText').innerHTML = 'Retrieved Query Definition for: "'
						+ dObj.name + '"';
				...
				for ( var pi = 0; pi < dm.panels.length; pi++) {
					// create a treeview root node and connect it to the
					// treeview controller
					dm.panels[pi].tvRootNode = new YAHOO.widget.RootNode(
							qpc.yuiTree);
					qpc.yuiTree.root = dm.panels[pi].tvRootNode;
					dm.panels[pi].tvRootNode.tree = qpc.yuiTree;
					qpc.yuiTree.setDynamicLoad(
							i2b2.CRC.ctrlr.QT._loadTreeDataForNode, 1);
					// load the treeview with the data
					var tvRoot = qpc.yuiTree.getRoot();
					for ( var pii = 0; pii < dm.panels[pi].items.length; pii++) {
						var withRenderData = qpc._addConceptVisuals(
								dm.panels[pi].items[pii], tvRoot, true);
		...
		// AJAX CALL
		i2b2.CRC.ajax.getRequestXml_fromQueryMasterId("CRC:QueryTool", {
			qm_key_value : qm_id
		}, scopedCallback);
	}
	...
	this.doQueryRun = function() {
		...
		var handleSubmit = function() {
			...
			if (this.submit()) {
				...
				var t3 = t.select('INPUT.chkDataCat');
				for ( var i = 0; i < t3.length; i++) {
					options['chk_' + t3[i].value] = t3[i].checked;
				}
		...
		var myDate = new Date();
		//Use local time in the default query name instead of UTC time
		//var ds = myDate.toUTCString();
		//var ds = myDate.toLocaleString(); //this doesn't work well due to browser inconsistency
//		var ts = ds.substring(ds.length - 4, ds.length - 12);
		var ts = myDate.getHours()+":"+myDate.getMinutes()+":"+myDate.getSeconds();
	...
	// ==================================================================================================
	this.toggleQueryType = function() {
		if ($('dataQuery').checked) {
		    $('countDatasources').hide();
		    $('dataDatasources').show();
		}
		if ($('countQuery').checked){
		   	$('countDatasources').show();
		    $('dataDatasources').hide();
		}
	}
	...
	this._queryRun = function(inQueryName, options) {
		...
		*/
		...
		iqst += ' Elapsed time (seconds): <span id="numSeconds">0</span><br />';
		$('infoQueryStatusText').innerHTML = iqst;
		this.queryIsRunning = true;
		var d = new Date();
		queryStartTime = d.getTime();
		...
		scopedCallback.scope = this;
		scopedCallback.callback = function(results) {
			var cl_query_definition = query_definition;
			var cl_query_name = inQueryName;
			// THIS function is used to process the AJAX results of the getChild
			// call
			// results data object contains the following attributes:
			// refXML: xmlDomObject <--- for data processing
			...
			i2b2.CRC.view.QT.queryResponse = results.msgResponse;
			try {
				var qi = results.refXML.getElementsByTagName('query_result_instance')[0];
				var QT = i2b2.CRC.ctrlr.QT;
				QT.doSetQueryName.call(QT, QT.queryMsg.queryName);
				
				// Old i2b2 code
				/*
				var set_size = i2b2.h.getXNodeVal(qi, 'set_size');
				var s = 'Query Finished...<br />Matching patients: ' + set_size
						+ '<br />';
				if (i2b2.PM.model.shrine_domain) {
					s = 'Query Finished...<br />';
					var ri = results.refXML
							.getElementsByTagName('query_result_instance');
					for ( var r = 0; r < ri.length; r++) {
						set_size = i2b2.h.getXNodeVal(ri[r], 'set_size');
						s += 'Matching patients (hospital ' + (r + 1) + '): '
								+ set_size + '<br />';
					}
				}
				*/
...
/*				s = 'Query Finished...<br />'; */
				...
				for ( var r = 0; r < joins.length; r++) {
					name = i2b2.h.getXNodeVal(joins[r], 'name');
					count = i2b2.h.getXNodeVal(joins[r], 'count');
					if (name == 'Sum') {
						s += 'Patient ' + name + ': ' + count + '<br />';
					} else if (name == 'Union') {
						s += 'Unique Patient Total: ' + count + '<br />';
					} else if (name == 'Intersection') {
						s += 'Patients Common in All Sources: ' + count + '<br />';
					}
				}
                                s += '</span>';
				// End FURTHeR code
				
				$('infoQueryStatusText').innerHTML += s;
				i2b2.CRC.ctrlr.history.Refresh();
				i2b2.CRC.ctrlr.QT.queryIsRunning = false;
				i2b2.CRC.ctrlr.QT.queryIsDirty = false;
			} catch (e) {
				$('infoQueryStatusText').innerHTML += "<div style='color:#F00'>An error has occurred while trying to run the query.<br />"
		...
		if (options.chk_PRS) {
			i++;
			result_output += '<result_output priority_index="' + i
					+ '" name="patientset"/>';
		}
		if (options.chk_PRC) {
			i++;
		...
		if (options.chk_data_query || options.chk_count_query) {
			...
			further_config += '<fqe-process>true</fqe-process>';
			...
			if (options.chk_data_query) {
				...
				if(options.chk_demographic)
					dc+='<datacategory>Demographics</datacategory>';
				if(options.chk_condition)
					dc+='<datacategory>Condition</datacategory>';
				if(options.chk_procedure)
			...
			if (ds != "") {
				ds = '<datasources>' + ds + '</datasources>';
				further_config += ds;
			}
			
			if(dc!=""){
				dc='<datacategories>'+dc+'</datacategories>';
				further_config += dc;
			}
			
			var as = "";	
			if (i2b2.CRC.model.associatedResults) {
				as += '<associatedResults>' + i2b2.CRC.model.associatedResults.origData.QI_id + '</associatedResults>';
			}
			...
			result_output += '<further-config>' + further_config
					+ '</further-config>';
		}
		params.psm_result_output = '<result_output_list>' + result_output
				+ '</result_output_list>';
		i2b2.CRC.ajax.runQueryInstance_fromQueryDefinition("CRC:QueryTool",
				params, scopedCallback);
	}
	...
	this._queryPromptRun = function(handleSubmit) {
		if (!i2b2.CRC.view.dialogQryRun) {
			...
			i2b2.CRC.view.dialogQryRun = new YAHOO.widget.SimpleDialog(
					"dialogQryRun", {
						width : "610px",
						fixedcenter : true,
			...
			i2b2.CRC.view.dialogQryRun.validate = function() {
				...
				var msgError = '';
				var queryNameInput = $('dialogQryRun').select(
						'INPUT.inputQueryName')[0];
	...
	this._queryPromptName = function(handleSubmit) {
		if (!i2b2.CRC.view.dialogQmName) {
			...
			i2b2.CRC.view.dialogQmName = new YAHOO.widget.SimpleDialog(
					"dialogQmName", {
						width : "400px",
						fixedcenter : true,
						constraintoviewport : true,
	...
	this._getQueryXML = function(queryName) {
		...
		if (panel_cnt > 0) {
			...
			if (auto_query_name_len < 1) {
				auto_query_name_len = 1;
			}
		}
		// build Query XML
		...
		for ( var p = 0; p < panel_cnt; p++) {
			...
			if (panel_list[p].dateFrom) {
				s += '\t\t<panel_date_from>' + panel_list[p].dateFrom.Year
						+ '-' + padNumber(panel_list[p].dateFrom.Month, 2)
						+ '-' + padNumber(panel_list[p].dateFrom.Day, 2)
						+ 'Z</panel_date_from>\n';
			}
			if (panel_list[p].dateTo) {
				s += '\t\t<panel_date_to>' + panel_list[p].dateTo.Year + '-'
						+ padNumber(panel_list[p].dateTo.Month, 2) + '-'
						+ padNumber(panel_list[p].dateTo.Day, 2)
						+ 'Z</panel_date_to>\n';
			}
			...
			// Occurs constraint
			s += '\t\t<total_item_occurrences>'
					+ ((panel_list[p].occurs * 1) + 1)
			...
			for (i = 0; i < panel_list[p].items.length; i++) {
				...
				s += '\t\t\t<item_name>' + sdxData.origData.name
						+ '</item_name>\n';
				s += '\t\t\t<item_key>' + sdxData.origData.key
						+ '</item_key>\n';
				s += '\t\t\t<tooltip>' + sdxData.origData.tooltip
						+ '</tooltip>\n';
				s += '\t\t\t<class>ENC</class>\n';
				s += '\t\t\t<constrain_by_date>\n';
				if (panel_list[p].dateFrom) {
					s += '\t\t<date_from>' + panel_list[p].dateFrom.Year + '-'
							+ padNumber(panel_list[p].dateFrom.Month, 2) + '-'
							+ padNumber(panel_list[p].dateFrom.Day, 2)
							+ 'Z</date_from>\n';
				}
				if (panel_list[p].dateTo) {
					s += '\t\t<date_to>' + panel_list[p].dateTo.Year + '-'
							+ padNumber(panel_list[p].dateTo.Month, 2) + '-'
							+ padNumber(panel_list[p].dateTo.Day, 2)
							+ 'Z</date_to>\n';
				}
				s += '\t\t\t</constrain_by_date>\n';
				s += '\t\t\t<item_icon>' + sdxData.origData.hasChildren
						+ '</item_icon>\n';
				try {
					var t = i2b2.h.XPath(sdxData.origData.xmlOrig,
							'descendant::synonym_cd/text()');
					t = (t[0].nodeValue == "Y");
				} catch (e) {
					var t = "false";
				}
				...
				// FUR-271, FUR-465
				// Set lvd = lab value data for the current panel and item
				// iterants.
				// It seems that sdx.LabValues has the most recent value of the
				// lab
				// after UI actions, and sdx.origData.LabValues is the value
				// before
				// all UI actions, e.g. the stored query value. Give preference
				// to
				// the former, if it exists; otherwise use the latter.
				var lvd = sdxData.LabValues;
				if (!lvd) {
					lvd = sdxData.origData.LabValues;
				}
				// alert('Orig ' + sdxData.origData.LabValues + ' New ' +
				// sdxData.LabValues + " lvd " + lvd);
				if (lvd) {
					...
					case "FLAG":
						...
						s += '\t\t\t\t<value_constraint>'
								+ i2b2.h.Escape(lvd.ValueFlag)
								+ '</value_constraint>\n';
						break;
					case "VALUE":
						s += '\t\t\t\t<value_type>' + lvd.GeneralValueType
								+ '</value_type>\n';
						if (lvd.GeneralValueType == "ENUM") {
							...
							s += '\t\t\t\t<value_constraint>' + sEnum
									+ '</value_constraint>\n';
							s += '\t\t\t\t<value_operator>IN</value_operator>\n';
						} else {
							s += '\t\t\t\t<value_unit_of_measure>'
									+ lvd.UnitsCtrl
									+ '</value_unit_of_measure>\n';
							s += '\t\t\t\t<value_operator>' + lvd.NumericOp
									+ '</value_operator>\n';
							if (lvd.NumericOp == 'BETWEEN') {
								s += '\t\t\t\t<value_constraint>'
										+ i2b2.h.Escape(lvd.ValueLow) + ' and '
										+ i2b2.h.Escape(lvd.ValueHigh)
										+ '</value_constraint>\n';
							} else {
								s += '\t\t\t\t<value_constraint>'
										+ i2b2.h.Escape(lvd.Value)
										+ '</value_constraint>\n';
							}
						}
						break;
				...
				s += '\t\t</item>\n';
				if (i == 0) {
					auto_query_name += sdxData.origData.name.substring(0,
							auto_query_name_len);
					if (p < panel_cnt - 1) {
						auto_query_name += '-';
					}
	...
	this.panelAdd = function(yuiTree) {
		// this function is used to create a new panel, it initializes the data
		// structure in the
	...
	this.ToggleNode = function(divTarg, divTreeID) {
		...
		var tvTree = YAHOO.widget.TreeView.findTreeByChildDiv(divTarg); // this
																		// is a
																		// custom
																		// extention
																		// found
	...
	this.panelDelete = function(index) {
		...
		var dm = i2b2.CRC.model.queryCurrent;
		if (index < 0 || index >= dm.panels.length) {
			return false;
		}
		dm.panels.splice(index, 1);
		// redraw the panels
		this.doShowFrom(this.panelControllers[0].panelCurrentIndex);
		// BUG FIX: force the panels to fully reattach the yuiRootNode to the
		// controllers
	...
	this.doShowFrom = function(index_offset) {
		var dm = i2b2.CRC.model.queryCurrent;
		var inclusionStr = " - Inclusions";
		var exclusionStr = " - Exclusions";
		// have all panel controllers redraw using new index offest
		if (index_offset === false) {
			return true;
		}
		if (index_offset < 0) {
			index_offset = 0;
		...
		for ( var i = 0; i < 3; i++) {
			this.panelControllers[i].refTitle.innerHTML = "Group "
					+ (index_offset + i + 1);
			this.panelControllers[i].setPanelRecord(index_offset + i);
			var panel = dm.panels[index_offset + i];
			if (undefined != panel) {
				if (panel.exclude) {
					this.panelControllers[i].refTitle.innerHTML += exclusionStr;
				} else {
					this.panelControllers[i].refTitle.innerHTML += inclusionStr;
				}
			}
			if (i > 0) {
	...
	this.doScrollNext = function() {
		var i = this.panelControllers[0].panelCurrentIndex + 1;
		var dm = i2b2.CRC.model.queryCurrent;
		if (i > (dm.panels.length - 3)) {
			i = dm.panels.length - 3;
		}
		this.doShowFrom(i);
	}
	...
	this.doAssociatedResultsDrop = function(sdxConcept) {
    	// save the info to our local data model
		i2b2.CRC.model.associatedResults = sdxConcept;
    	// let the user know that the drop was successful by displaying the name of the patient set
    	$("assocResultBox").innerHTML = i2b2.h.Escape(sdxConcept.sdxInfo.sdxDisplayName);
	}
}
