...
i2b2.ONT.view.find.PopulateCategories = function() {		
	...
	for (var i=0; i<i2b2.ONT.model.Categories.length; i++) {
		var cat = i2b2.ONT.model.Categories[i];
                // Display only FURTHeR-related ontologies (FUR-292, FUR-329)
                // We adopt the convention of displaying only categories whose TABLE_ACCESS
                // C_HLEVEL column value is non-negative. This is translated into the
                // catData.level field in the web client.
                if (cat.level >= 0)
                {
			...
			// Make the FURTHeR ontology (assumed to be the first category)
			// the default value. A temporary fix for FUR-204/FUR-292.
			if (i == 0)
			{
				tno.setAttribute('selected', 'true');
			}
