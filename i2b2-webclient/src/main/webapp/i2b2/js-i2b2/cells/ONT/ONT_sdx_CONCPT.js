...
// This would ensure removal of the onmouseover call in a cross-browser way
	...
	if (sdxData.origData.hasChildren == 'CA') {
		...
		bCanExp = true;
		//*********************************************************************************
		// Added by Dustin Schultz for FURTHeR 02/1/11
		//*********************************************************************************
	} else if (sdxData.origData.hasChildren == 'CAI') {
		// render as category
		icon = 'root';
		sDD = '';
		sIG = ' isGroup="Y"';
		...
		//*********************************************************************************
	} else if (sdxData.origData.hasChildren == 'FAI') {
		icon = 'branch';
		bCanExp = true;
		info = true;
		//*********************************************************************************
	...
	//*********************************************************************************
	// Modified by Dustin Schultz for FURTHeR 02/1/11
	//*********************************************************************************
	// **** Render the HTML ***
	var retHtml = '<DIV id="' + id + '" ' + sMainEvents + ' style="white-space:nowrap;cursor:pointer;float:left;">';
	retHtml += '<DIV ';
	...
	retHtml += '</DIV></DIV>';
	// Only add to top level elements
	if (info) {
		retHtml += '<div style="float: right;padding-left:5px;padding-top:2px;" onclick="cncpt.information(\'' + sdxData.origData.name + '\');"><img src="js-i2b2/cells/ONT/assets/info-icon.png"></div>'
	}
	render.html = retHtml;
	render.htmlID =  id;
	//*********************************************************************************
...
// TODO: REFACTOR THIS! (Roll into COMM message sniffer?)
		...
		for(var i=0; i<1*c.length; i++) {
			...
			// Bug, docs say it supports up to 3 chars
...
//*********************************************************************************
// Added Functions by Dustin Schultz 01/21/11
//*********************************************************************************

cncpt = i2b2.sdx.TypeControllers.CONCPT;

cncpt.information = function(concept) {
	var element = "dialogOntInfo";
	if (!i2b2.CRC.view.dialogInfo) {
		i2b2.CRC.view.dialogInfo = new YAHOO.widget.Panel(element, {
			draggable : true,
			zindex : 10000,
			width : "400px",
			autofillheight : "body",
		...
		$(element).show();
		i2b2.CRC.view.dialogInfo.render(document.body);
	}
	var resource = concept.toLowerCase().replace(/\s/g, '');
	$(element + "-viewer-body").update("Loading...");
	i2b2.CRC.view.dialogInfo.center();
	i2b2.CRC.view.dialogInfo.show();
	i2b2.FUR.ajax.GetDatasourceInfo("FUR:ONT:Help", {
		resourceName : resource + ".xml"
	}, function(results) {
		$(element + "-viewer-body").update(results.msgResponse);
	});
	// display the dialoge
	i2b2.CRC.view.dialogInfo.show();
	cncpt.stopEvent(window.event);
}

//*********************************************************************************
// Stop an event from propogating out to other elements.
//*********************************************************************************
cncpt.stopEvent = function(e) {
