...
/*
// setup key capture to activate on CTRL (then) F12 key stroke
i2b2.hive.MsgSniffer.__keyHandler = function(evt) {
	var selfRef = i2b2.hive.MsgSniffer;
	var keyCode = YAHOO.util.Event.getCharCode(evt);
	if (keyCode==123) {
		// F12 button is now down
		if (selfRef.crtlDown) {
			selfRef.crtlDown = false;
		...
		if (evt.type=="keydown") {
			// CTRL button is now down
			selfRef.crtlDown = true;
		} else {
...
YAHOO.util.Event.addListener(document, "keydown", i2b2.hive.MsgSniffer.__keyHandler);
YAHOO.util.Event.addListener(document, "keyup", i2b2.hive.MsgSniffer.__keyHandler);
*/