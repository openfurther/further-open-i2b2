...
i2b2.h.XPathDefaultNs = function(xmlDoc, xPath, namespace) {
	var retArray = [];
	if (!xmlDoc) { 
		console.warn("An invalid XMLDoc was passed to i2b2.h.XPath");
		return retArray;
	...
	try {
		if (window.ActiveXObject) {
			...
			try {  
				...
				try {
					xmlDoc.ownerDocument.setProperty("SelectionLanguage", "XPath");
					xmlDoc.ownerDocument.setProperty("SelectionNamespaces",
						    "xmlns:default='" + namespace + "'");
				} catch(e) {}
			}
			retArray = xmlDoc.selectNodes(xPath);
			...
			var ownerDoc = xmlDoc.ownerDocument;
			if (!ownerDoc) {ownerDoc = xmlDoc; }
			var nodes = ownerDoc.evaluate(xPath, xmlDoc, function resolver() { return namespace }, XPathResult.ANY_TYPE, null);
			var rec = nodes.iterateNext();
			while (rec) {
				retArray.push(rec);
				rec = nodes.iterateNext();
...
//Convert a string s to a DOM object.
i2b2.h.string2Dom = function(s)
{
        var tmp = document.createElement("dummyroot");
        tmp.innerHTML = s;
        var doc = tmp.firstChild;
        return doc;
};

//Convert a DOM to string and back to a DOM object.
// Removes the XML preamable from the serialized string because it
// seems to be more reliable in creating the output DOM.
i2b2.h.dom2String2Dom = function(doc)
{
        var serializer = new XMLSerializer();
        var s = serializer.serializeToString(doc);
        s = s.replace('<\?xml version="1.0" encoding="UTF-8" standalone="yes"\?>\r\n', '');
        var tmp = i2b2.h.string2Dom(s);
        return tmp;
};
...
//		mn.style.cursor = 'wait';
		mn.show();
		if (typeof(i2b2.PM.view.modal.login) != 'undefined' && i2b2.PM.view.modal.login != null)
		{
