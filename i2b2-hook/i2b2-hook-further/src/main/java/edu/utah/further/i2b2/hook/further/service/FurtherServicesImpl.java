/*******************************************************************************
 * Source File: FurtherServicesImpl.java
 ******************************************************************************/
...
final class FurtherServicesImpl implements FurtherServices
{
	...
	/**
	 * The URL of the FURTHeR infrastructure of where the send the i2b2 request XML.
	 */
	private String queryRequestUrl;

	/**
	 * The URL of the FURTHeR infrastructure of where to request current state of a query.
	 */
	private String queryStateUrl;

	/**
	 * The URL of the FURTHeR infrastructure of where to get result information about the
	 * query.
	 */
	 ...
	 * @param queryStateUrl
	 */
	public FurtherServicesImpl(final String queryRequestUrl, final String queryStateUrl)
	{
		super();
	 ...
	 * 
	 * @see
	 * edu.utah.further.i2b2.hook.further.service.FurtherService#i2b2QueryRequest(java
	 * .lang.String)
	 */
	public QueryContextIdentifier i2b2QueryRequest(final String i2b2RequestXml,
			final long i2b2QueryId)
	{
		final String queryRequestUrlWithId = queryRequestUrl + "/" + i2b2QueryId;
		if (log.isDebugEnabled())
		{
			log.debug("Sending request " + i2b2RequestXml + " to FURTHeR URL "
					+ queryRequestUrlWithId);
		}
		final HttpMethod method = HttpUtil.createPostMethod(queryRequestUrlWithId,
				i2b2RequestXml);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.utah.further.i2b2.hook.further.service.FurtherService#getQueryState(edu.
	...
	public QueryContextState getQueryState(final QueryContextIdentifier id)
	{
		...
		try
		{
			response = HttpUtil.getHttpGetResponseBody(queryStateUrlWithId, 3000);
		}
		catch (final WsException e)
		{
			throw new RuntimeException("Error getting query state for id " + id.getId(),
					e);
		}
		...
		try
		{
			...
			state = xmlService.unmarshal(responseXml, QueryContextStateTo.class);
		}
		catch (final Exception e)
		{
			throw new RuntimeException("Error unmarshalling query state response", e);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.utah.further.i2b2.hook.further.service.FurtherServices#getQuery(edu.utah
	 * .further.fqe.ds.api.to.QueryContextIdentifier)
	 */
	public QueryContextTo getQuery(final QueryContextIdentifier id)
	{
		...
		try
		{
			response = HttpUtil.getHttpGetResponseBody(queryResultsUrlWithId, 3000);
		}
		catch (final WsException e)
		{
			throw new RuntimeException("Error getting query for id " + id.getId(), e);
		}
		...
		try
		{
			...
			queryContextTo = xmlService.unmarshal(responseXml, QueryContextToImpl.class);
		}
		catch (final Exception e)
		{
			throw new RuntimeException("Error unmarshalling query from response", e);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.utah.further.i2b2.hook.further.service.FurtherService#getQueryRequestUrl()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.utah.further.i2b2.hook.further.service.FurtherService#setQueryRequestUrl
	 * (java.lang.String)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.utah.further.i2b2.hook.further.service.FurtherService#getQueryStateUrl()
	 */
	 ...
	 * 
	 * @see
	 * edu.utah.further.i2b2.hook.further.service.FurtherService#setQueryStateUrl(java
	 * .lang.String)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see edu.utah.further.i2b2.hook.further.service.FurtherServices#setQueryResultsUrl
	 * (java.lang.String)
	 */
	 ...
	 * 
	 * @see
	 * edu.utah.further.i2b2.hook.further.service.FurtherServices#getResultMaskBoundary ()
	 */
	 ...
	 * 
	 * @see
	 * edu.utah.further.i2b2.hook.further.service.FurtherServices#setResultMaskBoundary
	 * (java.lang.Long)
	 */
	...
	public String buildQueryResultXml(final QueryContextTo queryContextTo)
	{
		...
		// s.append("<set_size>" + queryContextTo.getNumRecords() + "</set_size>").append(
	 ...
	 * @param queryContext
	 *            FQC
	 * @return i2b2 TO graph ready to be marshaled and inserted into an I2b2 query
	 *         response message
	 */
			...
			final QueryContextTo queryContext)
	{
		...
		// Copy result context list to join result list. Not needed if there's only one
		// data source.
		if (numDataSources > 1)
		{
			final List<I2b2FurtherJoinResult> joinResults = queryResult.getJoins();
			final SortedMap<ResultContextKey, ResultContext> resultViews = CollectionUtil
					.newSortedMap();
			resultViews.putAll(queryContext.getResultViews());
			for (final Map.Entry<ResultContextKey, ResultContext> entry : resultViews
					.entrySet())
			{
				final ResultContextKey key = entry.getKey();
				final ResultContext resultContext = entry.getValue();
				joinResults.add(new I2b2FurtherJoinResult(getResultContextKeyString(key,
						numDataSources),
						isMasked ? (key.getType().equals(ResultType.SUM) ? resultContext
	 ...
	 */
	public String marshalI2b2FurtherQueryResult(final I2b2FurtherQueryResultTo queryResult)
	{
		...
		try
		{
			marshalled = xmlService.marshal(
					queryResult,
					xmlService
							.options()
							.setFormat(true)
							.addClass(I2b2FurtherQueryResultTo.class)
							.buildContext()
							.setMarshallerConfig(marshallerConfig)
							.setRootNamespaceUris(CollectionUtil.<String> newSet()));
			// Remove trailing space that JAXB inserts instead of the top XML declaration
		...
		marshalled = marshalled.replace(Integer.toString(ResultContext.ACCESS_DENIED),
				"Unauthorized");
		return marshalled;
	}
	...
	public long extractI2b2QueryId(final byte[] bytes)
	{
		...
		final String text = getNodeSubTreeAsString(new ByteArrayInputStream(bytes),
				"//query_instance_id/text()", NS_CONTEXT);
		return (text == null) ? INVALID_VALUE_LONG : Long.parseLong(text.trim());
	}

	// ========================= PRIVATE METHODS ===========================

	/**
	 * Create a textual representation of a result context key.
	 * 
	 * @param key
	 *            result context key
	 * @param numDataSources
	 *            number of responding data sources
	 * @return textual representation
	 */
	private static String getResultContextKeyString(final ResultContextKey key,
			final int numDataSources)
	{
		switch (key.getType())
		{
			case SUM:
			...
			case INTERSECTION:
			{
				final int intersectionIndex = key.getIntersectionIndex().intValue();
				if (intersectionIndex == 1)
				{
					return "Union";
				}
				else if (intersectionIndex == numDataSources)
				{
					return "Intersection";
				}
				else
				{
					return intersectionIndex + "-intersection";
				}
			}
	 ...
	 * @return
	 */
	private QueryContextIdentifierTo toQcIdentifier(final HttpResponseTo furtherResponse)
	{
		QueryContextIdentifierTo qcIdTo;
		try
		{
			qcIdTo = xmlService.unmarshal(furtherResponse.getResponseBodyAsString(),
					QueryContextIdentifierTo.class);
		}
		catch (final Exception e)
		{
			throw new RuntimeException("Error unmarshalling query request response", e);
		}
