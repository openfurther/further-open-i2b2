...
public class GetChildrenDao extends JdbcDaoSupport  {
	 ...
	 * @see edu.harvard.i2b2.ontology.api.IGetChildrenDao#findChildrenByParent(edu.harvard.i2b2.ontology.datavo.vdo.GetChildrenType, java.util.List)
	 */
	public List<ConceptType> findChildrenByParent(final GetChildrenType childrenType, final List<ConceptType> categories) throws DataAccessException{
...
// Lookup to get chlevel + 1 ---  dont allow synonyms so we only get one result back
		...
		String hidden = "";
		if(childrenType.getHiddens() == false)
			hidden = " and c_visualattributes not like '_H%'";
	
		String synonym = "";
		if(childrenType.getSynonyms() == false)
